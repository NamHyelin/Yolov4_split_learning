...
class Yolo_dataset(Dataset):
    def __init__(self, label_path, cfg, train=True):
        ...
        truth = {}
        for filename in glob.glob(os.path.join(label_path, '*.txt')):
            filename=filename.replace('\\','/',5)
            with open(filename, 'r', encoding='utf-8') as f:
                truth[filename.split('/')[-1].split('.')[0]+'.bmp'] = []
                for line in f.readlines():
                    data = line.split(" ")
                    if data[0]==0:
                        print('class 0 has existed')
                        raise
                    ...
                    truth[filename.split('/')[-1].split('.')[0] + '.bmp'].append(data)
    ...
    def __getitem__(self, index):
        ...
        #
        # a = draw_box(out_img.copy(), out_bboxes1.astype(np.int32))
...
def get_image_id(filename:str) -> int:
    ...
    # >>> return int(lv+no)
    """
    lv= int(os.path.splitext(os.path.basename(filename))[0])
    return lv
...
if __name__ == "__main__":
    ...
    random.seed(2020)
    np.random.seed(2020)
    Cfg.dataset_dir = 'C:/Users/hyeli/Anaconda3/envs/hlnam/yolov4/data/dataset/Train/central'
