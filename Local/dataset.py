        ...
        for i, filename in enumerate(glob.glob(os.path.join(label_path, '*.txt'))):
            with open(filename, 'r', encoding='utf-8') as f:
                ...
                for line in f.readlines():
                    ...
                    xc= data[0]  #center x
                    yc= data[1]  #center y
                    data[0]= xc-data[2]/2  #left upper x= center x-half width
                    data[1]= yc-data[3]/2  #left upper y= center y-half height  #aa 더해야되는거 아닌가
                    data[2]= xc+data[2]/2
                    data[3]= yc+data[3]/2
                    data[0] *= cfg.width   # un-normalization
                    data[1] *= cfg.height
                    data[2] *= cfg.width
                    data[3] *= cfg.height
                    truth[str(i+1).zfill(3)+'.bmp'].append(data)
